...
const VOTE_FORM_PREFILL_URL = 'https://docs.google.com/forms/d/e/1FAIpQLSeUIJK235Llv81IRGQEEsTuA_wrUfKaILYAdiP2lmHWyyxddw/viewform?usp=pp_url&entry.538087045=';
...
function sitesPage(items,origin){
 const cards=items.length?items.map(item=>{const url=`${origin}/s/${encodeURIComponent(item.id)}/`,voteUrl=VOTE_FORM_PREFILL_URL+encodeURIComponent(item.team);return `<article class="site" data-card="${escapeHtml(item.id)}"><div><span class="eyebrow">TEAM SITE</span><h2>${escapeHtml(item.team)}</h2><p>${escapeHtml(url)}</p></div><div class="actions"><a class="open" href="${escapeHtml(url)}" target="_blank" rel="noopener">サイトを見る ↗</a><a class="vote" href="${escapeHtml(voteUrl)}" target="_blank" rel="noopener">このチームに投票</a><button data-url="${escapeHtml(url)}">URLコピー</button><button class="delete" data-delete="${escapeHtml(item.id)}" data-team="${escapeHtml(item.team)}" hidden>削除</button></div></article>`}).join(''):'<div class="empty">まだ公開されたサイトはありません。</div>';
 ...
 :root{--red:#ed1c24;--ink:#090909;--paper:#f4f2ed}*{box-sizing:border-box}[hidden]{display:none!important}body{margin:0;min-height:100vh;padding:24px;font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI","Noto Sans JP",sans-serif;background:var(--paper);color:var(--ink)}.wrap{width:min(1080px,100%);margin:auto}header{display:flex;justify-content:space-between;align-items:center;padding:8px 0 22px;border-bottom:1px solid #aaa}.brand{font-weight:950;font-size:20px;letter-spacing:-.04em}.brand b{display:inline-grid;place-items:center;width:28px;height:28px;margin-right:10px;background:var(--red);color:#fff;font-size:12px;transform:rotate(-5deg)}header a{color:#111;font-size:13px;font-weight:800;text-decoration:none}h1{font-size:clamp(42px,8vw,74px);letter-spacing:-.07em;line-height:.95;margin:56px 0 12px}h1 span{color:var(--red)}.lead{color:#666;margin:0 0 38px}.grid{display:grid;gap:15px}.site{display:flex;align-items:center;justify-content:space-between;gap:20px;background:#fff;border:1px solid #bbb;padding:24px;box-shadow:7px 7px 0 #111;transition:.2s}.site>div:first-child{min-width:0}.site.removing{opacity:0;transform:translateX(20px)}.eyebrow{font:800 10px monospace;letter-spacing:.14em;color:var(--red)}h2{font-size:26px;margin:5px 0 8px;letter-spacing:-.04em}.site p{max-width:570px;margin:0;color:#777;font:12px/1.5 monospace;word-break:break-all}.actions{display:grid;grid-template-columns:repeat(2,max-content);gap:8px;flex-shrink:0}.actions a,.actions button{border:0;padding:12px 16px;text-align:center;text-decoration:none;font-size:12px;font-weight:900;cursor:pointer}.actions .open{background:#111;color:#fff}.actions .vote{background:var(--red);color:#fff}.actions button{background:#dedbd5;color:#111}.actions .delete{background:#fff;color:#b0000b;border:1px solid #b0000b}.empty{padding:50px;background:#fff;border:1px solid #bbb;text-align:center;color:#777}.toast{position:fixed;right:20px;bottom:20px;padding:13px 17px;background:#111;color:#fff;font-size:13px;font-weight:800;transform:translateY(160%);transition:.25s}.toast.show{transform:none}@media(max-width:760px){body{padding:16px}.site{display:block;padding:19px;box-shadow:5px 5px 0 #111}.actions{margin-top:16px;grid-template-columns:repeat(2,1fr)}.actions a,.actions button{width:100%}h1{margin-top:42px}}@media(max-width:420px){.actions{grid-template-columns:1fr}}
 </style></head><body><main class="wrap"><header><div class="brand"><b>V</b>VOICE PUBLISH</div><a href="/">＋ サイトを作る</a></header><h1>みんなの<span>Webサイト。</span></h1><p class="lead">サイトを見て、応援したいチームに投票してください。投票は1人1回です。</p><section class="grid">${cards}</section></main><div id="toast" class="toast" role="status"></div><script>
